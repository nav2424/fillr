...
const CONTAINS_PATTERNS = [
  ...
  // Require a colon so Slovak "neobsahuje" / "ne obsahuje" is not stolen,
  // and so "môže obsahovať" can be classified as may-contain first.
  { re: /\bobsahuje\s*:\s*/i, name: 'obsahuje' },
  { re: /allergens?\s*:?\s*/i, name: 'allergens' },
  { re: /allergènes?\s*:?\s*/i, name: 'allergenes' },
]

const INGREDIENTS_PREFIX = /^(?:ingredients?|zloženie|zlozenie)\s*:?\s*/i
...
const MAY_CONTAIN_PATTERNS = [
  ...
  /môže\s+obsahovať\s*:?\s*/i,
  /moze\s+obsahovat\s*:?\s*/i,
