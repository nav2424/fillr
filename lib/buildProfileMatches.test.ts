...
test('goalConflictDetails lists ingredient lines that matched each conflict', () => {
  ...
  assert.equal(muscle?.ingredients.length ?? 0, 0, 'product-scoped protein goal has no per-line conflicts')
  assert.ok(result.goalConflicts.includes('Eat more protein'), 'product goal still affects score')
