...
export function hasMeaningfulLocalDietProfile(
  ...
  if (!p) return false
  return p.allergies.length + p.sensitivities.length + p.avoiding.length + p.preferences.length > 0
}
...
/** Build a scan-ready dietary profile from current onboarding selections. */
