...
test('Dried oregano is always clean', () => {
  const result = applyDeterministicRatings([{ name: 'Dried oregano', rating: 'okay' }])
  assert.equal(result[0].rating, 'clean')
})

test('Dried rosemary is always clean', () => {
  const result = applyDeterministicRatings([{ name: 'Dried rosemary', rating: 'okay' }])
  assert.equal(result[0].rating, 'clean')
})
