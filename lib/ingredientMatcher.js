...
const ALWAYS_CLEAN = [
  ...
  'lemon juice',
  'lime juice',
  // Dried herbs & common seasonings — minimally processed single-ingredient pantry items.
  'oregano',
  'rosemary',
  'thyme',
  'basil',
...
function deterministicReason(nameNormalized, rating) {
  if (rating === 'concerning') {
    ...
    if (
      /calcium\s+disodium\s+edta|disodium\s+calcium\s+edta|disodium\s+edta|trisodium\s+edta|ethylenediaminetetraacetic/.test(
        nameNormalized
      )
    ) {
      return 'Synthetic chelating agent used to preserve color and slow oxidation. Regulators generally classify it as safe, but it is an industrial additive you would not use in a home kitchen.'
    }
    if (/sorbic\s+acid|potassium\s+sorbate/.test(nameNormalized)) {
      return 'Synthetic preservative in the sorbate family. Very common in shelf-stable condiments; stricter clean-label standards often treat it as something to limit.'
    }
    if (/modified\s+.*\bstarch|modified\s+corn\s+starch|modified\s+potato\s+starch|modified\s+maize\s+starch/.test(nameNormalized)) {
      return 'Chemically modified starch for thickness or emulsion stability. Normal in packaged foods, not something you would reproduce from scratch at home.'
    }
  }
  if (rating === 'okay' && /canola|rapeseed/.test(nameNormalized)) {
    return 'Highly refined seed oil and usually the largest ingredient by weight in oil-heavy formulas. Not acutely unsafe, but higher in omega-6 than olive or avocado oil.'
  }
...
function humanizeProfileKey(s) {
  const k = normalizeSlug(s)
  if (!k) return 'your saved setting'
  const labels = {
    lactose: 'Lactose',
    ...
    if (priority < 4) {
      for (const sensitivity of sensitivityTerms) {
        ...
        if (matched) {
          ...
          updated.personalMessage = `You've flagged sensitivity to ${humanizeProfileKey(sensitivity)}.`
          updated.ratingSource = 'personal'
          priority = 3
          break
        }
    ...
    if (priority < 2) {
      for (const pref of preferenceTerms) {
        ...
        if (matched) {
          ...
          updated.personalMessage = `This ingredient may not match your ${humanizeProfileKey(pref)} preference.`
          priority = 1
          break
        }
      }
